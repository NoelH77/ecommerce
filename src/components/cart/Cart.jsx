...
const Cart = () => {
  ...
  return (
    <section className={`fixed top-0 bg-white shadow-xl 
    h-screen w-[300px] ${isShowCart && token ? "right-0" : "-right-full"} 
    duration-200 p-3 grid grid-rows-[auto_1fr_auto]`}>
        <h2 className='text-xl font-bold'>Shopping cart</h2>
        <i onClick={handleClickChangeShowCart} className='bx bx-x absolute top-2 right-3 text-xl hover:text-red-500
        cursor-pointer'></i>
        ...
        <section className='grid grid-cols-2 py-10 border-t-[1px] border-gray-400'>
            ...
            <button onClick={handleClickCheckout} className="w-full col-span-2 bg-red-500 py-2 text-white hover:bg-red-600 
        transition-colors rounded-sm mt-6">Checkout</button>
        </section>
